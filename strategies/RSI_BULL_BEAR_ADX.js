...
var settings = config.RSI_BULL_BEAR_ADX;
...
var strat = {
	...
	init: function()
	{
		...
		// ADX
		this.addIndicator('ADX', 'ADX', settings.ADX );
		
		// MOD (RSI modifiers)
		this.BULL_MOD_high = settings.BULL_MOD_high;
		this.BULL_MOD_low = settings.BULL_MOD_low;
		this.BEAR_MOD_high = settings.BEAR_MOD_high;
		this.BEAR_MOD_low = settings.BEAR_MOD_low;
		...
		// warn users
		if( this.requiredHistory < settings.SMA_long )
		{
	...
	check: function()
	{
		...
		if( maFast < maSlow )
		{
			...
			let rsi_hi = settings.BEAR_RSI_high,
				rsi_low = settings.BEAR_RSI_low;
			
			// ADX trend strength?
			if( adx > settings.ADX_high ) rsi_hi = rsi_hi + this.BEAR_MOD_high;
			else if( adx < settings.ADX_low ) rsi_low = rsi_low + this.BEAR_MOD_low;
		...
		else
		{
			...
			let rsi_hi = settings.BULL_RSI_high,
				rsi_low = settings.BULL_RSI_low;
			
			// ADX trend strength?
			if( adx > settings.ADX_high ) rsi_hi = rsi_hi + this.BULL_MOD_high;		
			else if( adx < settings.ADX_low ) rsi_low = rsi_low + this.BULL_MOD_low;
