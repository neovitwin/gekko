...
var settings = config.customDEMA;
...
method.init = function() {
  ...
  this.addIndicator('ema', 'EMA', settings);
...
method.update = function(candle) {
  ...
  if(candle.close > this.dip.ATH)
  {
    ...
    if((dema.inner.age*this.candleStandarizer) > 10)
...
method.check = function(candle) {
  ...
  {  
    ...
    { 
      ...
      this.trend.direction = 'none';
      if(this.history.close[this.history.close.length-1] > this.history.close[this.history.close.length-2] )
      {
          if(this.history.close[this.history.close.length-1] > this.history.resDEMA[this.history.resDEMA.length-1])
          {
            console.log('up2');
            this.trend.direction = 'up';  
          }           
  ...
  // if(this.dip.active === true)
  // {
  //   console.log('Dip?', this.dip.ATH, ' , ',this.history.close[this.history.close.length-2], " , ", this.dip.noBuyZone );
  //   if(price < this.dip.ATH /*&& this.history.close[this.history.close.length-2] === this.dip.ATH */&& this.dip.noBuyZone === false && this.trend.direction === 'down')
  //   {
  ...
  //     if(((this.dip.rATH < price && this.dip.rATL*1.05 < price) || price >= this.dip.ATH ) && this.trend.direction === 'down')
  //     {
  ...
  if(!this.dip.noBuyZone)
  {
    if(this.trend.direction === 'up' && this.history.buyPrice === 0 && this.lastTransactionAge > 5)
    {
