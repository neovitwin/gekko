...
var settings = config.DUAL_RSI_BULL_BEAR;
...
method.init = function() {
  ...
  this.selector = this.addIndicator('selector', 'SMA', settings.selectorWeight);

   // here we init child strategies. 
   // take note that each one takes corresponding subdict of settings;
   this.RBB1 = this.createChild("RSI_BULL_BEAR", settings.RBB1);
   this.RBB2 = this.createChild("RSI_BULL_BEAR", settings.RBB2);
...
method.check = function(candle) {
    ...
    if (Selector > settings.selectorThreshold)
    {
