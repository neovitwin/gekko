...
var strat = {
	...
	init: function()
	{
		...
		this.resetTrend();
		this.addTulipIndicator('maSlow', 'tema', { optInTimePeriod: settings.long });
		this.addTulipIndicator('maFast', 'sma', { optInTimePeriod: settings.short });
		
		// check if long SMA is to be used
		if( settings.SMA_long > 0 )
		{
			this.useSafety = true;
			this.addTulipIndicator('maSlowest', 'sma', { optInTimePeriod: settings.SMA_long });
		}
